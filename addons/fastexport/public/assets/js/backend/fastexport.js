    ...
    var Controller = {
                            ...
                            buttons: [
                                {
                                    ...
                                    success: function (data, ret) {
                                        ...
                                        if (ret.code == 1 && data.direct_export) {
                                            window.location = 'fastexport/performSubTask/subtaskId/1/directExport/true/taskId/' + data.id;
                                        } else if (ret.msg) {
                                            Layer.alert(ret.msg);
                                        } else {
                                            Layer.alert('未知错误！');
                                    ...
                                    success: function (data, ret) {
                                        ...
                                        if (ret.code == 1) {
                                            if (data.direct_export) {
                                                ...
                                                window.open("fastexport/taskControl/taskId/" + data.id,"_blank");
                                            }
                                        } else {
                                            Layer.alert(ret.msg);
                                        }
        ...
        taskcontrol: function () {
            ...
            $(document).on('click', '.retry_task', function () {
                ...
                $.ajax({
                    url: "fastexport/performSubTask/taskId/" + Config.task.id + "/subtaskId/" + subtask_id,
                    dataType: "json",
        ...
        api: {
            formatter: {
                progress: function (value, row) {
                    ...
                    if (value >= 5) {
                        return '<a target="_blank" href="fastexport/taskControl/taskId/' + row.id + '" class="searchit ' + span_class + '" data-toggle="tooltip" title="" data-original-title="点击进入任务控制器">' + value + '%</a>';
                    } else {
            ...
            checkTask: function () {
                $.ajax({
                    url: "fastexport/taskFilePack/taskId/" + Config.task.id,
                    complete: function (ret) {
                        ret = ret.responseJSON;
                        try {
                            if (ret.code == 1) {
