...
function imgUrl($path)
{
    ...
    if (substr($path, 0, 4) == 'http') {
        return $path;
    } elseif (substr($path, 0, 10) == 'data:image') {
        return $path;
    } else {
...
function back($msg = "系统错误", $code = 0, $data = [], $acode = 200)
{
    header('Content-Type: application/json');
    http_response_code($acode);
    exit(json_encode([
        'code' => (int)$code,
        'msg' => $msg,
        'data' => $data
...
if (!function_exists('t')) {
    ...
    {
        $ULang = \request()->header('u-lang');
        if (empty($ULang))
            $ULang = 'zh';
        $file = ROOT_PATH . 'application/langs/' . $ULang . '.php';
        if (!file_exists($file)) {
            $file = ROOT_PATH . 'application/langs/zh.php';
        }
        $lang = include $file;
...
if (!function_exists('compareArrays')) {
     ...
     * @return bool
     */
    function compareArrays($arr1, $arr2): bool
    {
        // 如果数组长度不同，直接返回false
        ...
        foreach ($arr1 as $key => $value) {
            if (is_array($value) && is_array($arr2[$key])) {
                // 递归比较子数组
                if (!compareArrays($value, $arr2[$key])) {
                    return false;
                }
            }
        }
