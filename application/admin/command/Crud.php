...
class Crud extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        try {
            ...
            // 构建模型的属性
            $table_fields_data = array_column($columnList, "COLUMN_NAME");
            $table_fields = [
                "/**",
                "*"
            ];
            foreach ($table_fields_data as $vla) {
                $table_fields[] = sprintf("* @property array|bool|float|int|mixed|object|stdClass|null $%s", $vla,);
            }
            $table_fields[] = '*';
            $table_fields[] = "*/";
            $table_fields = join("\r\n", $table_fields);
            ...
            foreach ($columnList as $k => $v) {
                ...
                if ($v['COLUMN_KEY'] != 'PRI' && !in_array($field, $this->reservedField) && !in_array($field, $this->ignoreFields)) {
                    ...
                    if ($inputType == 'select') {
                        ...
                        //如果是图片或文件
                        if ($isUpload) {
                            $formAddElement = Form::input('hidden', $fieldName, $defaultValue, $attrArr);
                            $formEditElement = Form::input('hidden', $fieldName, $editValue, $attrArr);
